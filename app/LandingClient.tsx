...
const translations = {
  en: {
    ...
    paymentDesc: "USDT only. Contact support after payment for access.",
  ...
  zh: {
    ...
    paymentDesc: "接受 USDT。付完后找客服报备，即刻发货。",
    contactSupport: "联系客服",
    admin: "管理员"
  }
};
...
const getPlansList = (lang: Language, t: any, mult: number = 1) => [
  {
    name: t.planBasic,
    price: `$${200 * mult}`,
    period: "/mo",
...
export default function LandingClient({ brand }: { brand: BrandConfig }) {
  const { lang, mounted, toggleLanguage } = useLanguage();

  if (!mounted) return null; // Prevent hydration mismatch

  // 基础文案叠加当前品牌覆盖（title / hero / trialNote / badge 等按品牌切换）
  const t = { ...translations[lang], ...brand.locale[lang] };
  const featuresList = getFeaturesList(t);
  const plansList = getPlansList(lang, t, brand.priceMultiplier);

  // 把品牌主题作为 CSS 变量注入根节点，驱动下方所有 primary/渐变/辉光
  const themeStyle = {
    "--color-primary": brand.theme.primary,
    "--color-ring": brand.theme.ring,
    "--brand-accent-1": brand.theme.accent1,
    "--brand-accent-2": brand.theme.accent2,
    "--brand-glow": brand.theme.glow,
  } as CSSProperties;

  return (
    <div className="min-h-screen flex flex-col relative text-white selection:bg-primary/30" style={themeStyle}>
      ...
      <section className="flex flex-col items-center justify-center px-6 pt-32 pb-20 overflow-hidden relative">
        ...
        >
          ...
          <motion.h2 variants={fadeIn} className="text-5xl sm:text-6xl lg:text-7xl font-bold tracking-tight leading-[1.1]">
            ...
            <span className="block mt-2 bg-clip-text text-transparent bg-gradient-to-r from-[var(--brand-accent-1)] via-primary to-[var(--brand-accent-2)] pb-2">
          ...
          <motion.div variants={fadeIn} className="flex flex-col sm:flex-row gap-4 justify-center items-center pt-4">
            <Link href="/register" className="w-full sm:w-auto">
              <Button size="lg" className="h-12 px-8 w-full sm:w-auto text-base bg-gradient-to-r from-primary to-[var(--brand-accent-2)] hover:opacity-90 text-white shadow-[0_0_30px_rgba(var(--brand-glow),0.4)] hover:shadow-[0_0_40px_rgba(var(--brand-glow),0.6)] transition-all border-0 group">
                {t.startFreeTrial}
      ...
      <section id="pricing" className="px-6 py-32 border-t border-white/10 relative overflow-hidden">
        <div className="absolute top-0 right-1/4 w-[400px] h-[400px] bg-[rgba(var(--brand-glow),0.1)] rounded-full blur-[120px] pointer-events-none" />
              ...
              <motion.div key={plan.name} variants={fadeIn} className={plan.highlight ? "md:-mt-8 md:mb-8" : ""}>
                <Card
                  className={`relative flex flex-col h-full overflow-hidden transition-all duration-300 hover:-translate-y-2 ${plan.highlight
                    ? "bg-black/40 border-primary/50 shadow-[0_0_40px_rgba(var(--brand-glow),0.15)] backdrop-blur-xl"
                    : "bg-white/5 border-white/10 backdrop-blur-md"
                    }`}
                >
                  {plan.highlight && (
                    <>
                      <div className="absolute top-0 inset-x-0 h-1 bg-gradient-to-r from-[var(--brand-accent-1)] via-primary to-[var(--brand-accent-2)]" />
                    ...
                    <Link href="/register" className="w-full mt-auto">
                      <Button
                        className={`w-full h-12 text-base font-medium transition-all ${plan.highlight
                          ? "bg-primary hover:bg-primary/90 text-white shadow-[0_0_20px_rgba(var(--brand-glow),0.3)]"
      ...
      <footer className="border-t border-white/10 px-6 py-8 bg-black/40 backdrop-blur-md mt-auto relative z-10">
        <div className="max-w-6xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-2 text-white/40 text-sm">
            <span className="font-semibold text-white/50">{brand.footerName}</span>
