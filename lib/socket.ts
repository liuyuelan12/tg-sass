...
import {
  runAIChatJob,
  stopAIChatJob,
  getActiveAIChatJob as getActiveAIChatJobShared,
  isAIChatJobActive,
} from "./ai-chat/start";
import type { AIChatRunner } from "./telegram/ai-chat";

// In-memory store of active jobs (auto-chat + scrape still local; ai-chat moved to lib/ai-chat/start.ts)
...
export function getActiveAIChatJob(jobId: string): AIChatRunner | undefined {
  return getActiveAIChatJobShared(jobId);
}
